...
export default function Pratos() {
  ...
  const filteredPratos = pratos.filter(item => 
         ...
         <div className="bg-white border border-mesaninas-creme rounded-xl shadow-sm flex-1 w-full flex flex-col overflow-hidden">
          ...
          <thead className="bg-[#f4efdc]/30 text-[10px] uppercase tracking-wider font-bold text-[#00382b]/60 sticky top-0 z-10 shadow-sm">
            <tr className="border-b border-[#f4efdc]/50">
              <th className="w-16 px-6 py-3 font-semibold">Foto</th>
              <th className="px-6 py-3 font-semibold">Item</th>
              <th className="px-6 py-3 font-semibold text-center">Tipo de Venda</th>
              <th className="px-6 py-3 font-semibold text-center">Fornecedores</th>
          ...
          <tbody className="divide-y divide-mesaninas-creme/50">
            {loading ? (
              ...
              <tr>
                <td colSpan={5} className="px-6 py-12 text-center text-mesaninas-green/50 text-sm">
                  {searchTerm ? 'Nenhum item encontrado para a busca.' : 'Nenhum item cadastrado.'}
                </td>
              </tr>
            ) : (
              filteredPratos.map((prato) => (
                <tr key={prato.id} className="hover:bg-mesaninas-creme/30 group border-b border-mesaninas-creme/30">
