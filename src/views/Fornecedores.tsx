...
interface FornecedorFormModalProps {
  isOpen: boolean;
  onClose: () => void;
  fornecedorData?: Fornecedor | null;
  onSaveSuccess: () => void;
}

function FornecedorFormModal({ isOpen, onClose, fornecedorData, onSaveSuccess }: FornecedorFormModalProps) {
  ...
  useEffect(() => {
    if (fornecedorData) {
      ...
      setCpfCnpj(fornecedorData.cpf_cnpj || '');
      setEmail(fornecedorData.email || '');
      setTelefone(fornecedorData.telefone || '');
      setContatoPrincipal(fornecedorData.contatoPrincipal || '');
      setCep(fornecedorData.cep || '');
      setLogradouro(fornecedorData.logradouro || '');
      ...
      setObservacoes(fornecedorData.observacoes || '');
    } else {
      setNome('');
      setCpfCnpj('');
      setEmail('');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (fornecedorData?.id) {
        ...
        await updateDoc(fornecedorRef, payload);
      } else {
        ...
        <div className="px-6 py-4 border-b border-mesaninas-creme/50 flex justify-between items-center bg-white/50 shrink-0">
          <div>
            ...
            <p className="text-xs text-mesaninas-green/70">
              {fornecedorData ? 'Atualize as informações do fornecedor' : 'Cadastre um novo fornecedor'}
            </p>
          </div>
          <button
            onClick={onClose}
...
export default function Fornecedores() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        console.error("Erro de permissão ou conexão ao buscar fornecedores no Firebase:", err);
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'fornecedores', itemToDelete));
      showToast('Fornecedor apagado com sucesso.');
    } catch (err: any) {
      console.error("Erro ao apagar:", err);
      alert('Erro ao apagar fornecedor: ' + err.message);
    } finally {
  ...
  return (
    <div className="flex flex-col h-full relative gap-6">
      ...
      <div className="flex-1 min-h-0 flex flex-col">
          ...
          <tbody className="divide-y divide-mesaninas-creme/50">
            {loading ? (
              <tr>
               ...
               <tr>
                <td colSpan={5} className="px-6 py-12 text-center text-mesaninas-green/50 text-sm">Nenhum fornecedor encontrado.</td>
              </tr>
            ) : (
        ...
        <div className="lg:hidden flex flex-col p-4 gap-4 bg-mesaninas-creme/5">
            {loading ? (
              ...
              <div className="text-center text-mesaninas-green/50 text-sm py-8">Nenhum fornecedor encontrado.</div>
            ) : (
              filteredFornecedores.map((fornecedor) => (
                <div key={fornecedor.id} className="bg-white border border-mesaninas-creme/70 rounded-xl p-4 shadow-sm flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1.5 text-sm bg-mesaninas-creme/30 p-3.5 rounded-lg border border-mesaninas-creme/50">
                    <div className="flex items-center gap-2.5 text-mesaninas-green">
                      ...
                      <span className="font-medium">{fornecedor.telefone}</span>
                    </div>
      ...
      <FornecedorFormModal
        ...
        onSaveSuccess={() => showToast(editingFornecedor ? 'Fornecedor atualizado com sucesso!' : 'Novo fornecedor cadastrado com sucesso!')}
      />
